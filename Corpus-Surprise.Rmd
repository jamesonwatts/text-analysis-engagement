...
## Now let's isolate the documents that use the growth terms the most in 2004 
Note there are only 5 articles from that year, which may be driving the results.
...
for(i in 2005:2018){
  print(ggplot(
    ...
    top_n(25, abs_deviation) %>% 
    mutate(term = reorder(term, abs_deviation)),
    aes(x = term, y = deviation, fill=deviation)) + 
      geom_bar(stat = 'identity') + 
      scale_fill_gradient2(low='red', mid='snow3', high='darkgreen', space='Lab')+
      labs(x = NULL, y=NULL, title = str_c("Year: ",i))+
      coord_flip()
  )
}
```
